function [U,S,output] = lmlra_hooi_time(T,U0,normT, options)
...
if ~isfield(options,'TolFun'), options.TolFun = 1e-10; end
...
while ~output.info
    ...
    % curr_time = toc;
    % running_time = running_time + curr_time;


    S_iter = tensor(mat2tens(U{N}'*T1,size_core,N));

    T_pred = ttm(S_iter, U);
    error = norm(T - T_pred)^2;

    % Update the output structure.
    output.iterations = output.iterations+1;
    output.normS(end+1) = norm(s);
    output.normR(end+1) = error;
    output.time(end+1) = toc;
    % output.itertime(end+1) = running_time;
    output.sangle(end+1) = subspace(U{1},U1);

    reschange = abs(normR_old - error);

    output.info = reschange <= options.TolFun;
